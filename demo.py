...
parser.add_argument('--label_filter', type=bool, default=False,
                    help='using the entropy to filter the pseudo label')
parser.add_argument('--filter_steps',type=float, nargs=4, default=[1,2001,4001,6001],
                    help='the steps that filtered')
...
parser.add_argument('--message', type=str, default='Multi-JOINT with RCS divergence',
                    help='the annotation')   
...
if args.label_filter is True:
    record_file = os.path.join(record_dir, 'Multi_{}_{}_to_{}_with_labelFilter.txt'.format(args.net, args.source, args.target))
else:
...
if __name__ == '__main__':
    ...
    domain_labels.append(label) 
    dataloaders = {}
    model = RRL(args.net, class_num, bottleneck_dim, domain_labels, args.alpha_div, args.beta_div, args.lambda_div).to(DEVICE)

    with open(gl.get_value('record_file'), 'a') as f: 
        if args.message:
            f.write('\n' + args.message + '\n')
        f.write("alpha_div: %f, beta_div: %f lambda_div: %f\n"%(args.alpha_div, args.beta_div, args.lambda_div))
        f.write(" the gamma in the loss is %f \n"%args.gamma)
        f.write(" the init leraning rate is %f \n"%args.lr)
        f.write(" the preprocess type is %d \n"%args.pre_type)
    ...
    if args.phase == 'train':
        ...
        if args.dataset in ['fullDomainnet']:
            dataloaders['validation'] = load_data_for_MultiDA(
                args, args.root_dir, args.dataset, args.source, args.target, 'get_validation', batch_size, args.net, pre_type=args.pre_type)
            print("validation datasets sizes:")
            print(len(dataloaders['validation'].dataset))
            print("target test datasets sizes:")
            print(len(dataloaders['tar_test'].dataset))
        ...
        pseudo_labels = predict(model, dataloaders['tar_test'],with_filter=args.label_filter, filter_steps=args.filter_steps)
