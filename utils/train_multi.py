...
def train_for_Multi(args, model, optimizer, dataloaders, domain_labels):
    ...
    for step in range(1, args.steps + 1):
        ...
        s_output, div_loss = model(source=s_img, target=t_img, source_label=s_label, 
                                        source_domain_label=s_domain_label, target_domain_label=t_domain_label, domain_labels=domain_labels)
        ...
        if step > 0 and step % args.save_interval == 0:
            ...
            if step >= args.start_update_step and step % args.update_interval == 0 :
                ...
                for i in range(len(dataloaders['src_train_l_list'])):
                    # dataloaders['src_train_l_list'][i].batch_sampler.label_sampler.set_batch_num(args.update_interval)  
                    src_label_data_list[i] = iter(dataloaders['src_train_l_list'][i])
                pseudo_labels = predict(model, dataloaders['tar_test'],with_filter=args.label_filter, step=step, filter_steps=args.filter_steps)
            ...
            if args.early:
                if step >= 20000 and acc_tar > best_acc:
                    ...
                    torch.save(model.state_dict(), '{}/best_Multi_{}_{}_to_{}_alpha_{}_beta_{}_lambda_{}.pth'.format(check_path, args.net, args.source, args.target, args.alpha_div, args.beta_div, args.lambda_div))
                else:
                    counter += 1
